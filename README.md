...
- 基础训练完成后，网络在剔除了object365的数据集上进行样本均衡微调。损失函数的参数更改为gamma_pos=gamma_neg=1.5（即gamma=1.5的focal loss缩放）。学习率周期为10+20+40，从1e-4到1e-8余弦衰减。同时，将色相数据增强的强度从0.2提高到0.4，以增加对变色奶龙的识别能力。
...
- 2025.8.29 <br>
    * 完善README
    * 更新了yvgg网络，提高准确性。
    * 更改GUI多线程加载逻辑，将图像缩放任务放到并行线程内，增加吞吐量。
- 2025.8.21 <br>
    * 完善README
